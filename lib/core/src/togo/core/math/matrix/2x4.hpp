...
/** @cond INTERNAL */
template<class T>
struct TMat2x4<T>::operations {
	using type = typename TMat2x4<T>::type;
...
/** @name Comparison operators */ /// @{
	...
	template<class T>
	inline bool operator==(TMat2x4<T> const& m, TMat2x4<T> const& n) {
		return
			m.data[0] == n.data[0] &&
			m.data[1] == n.data[1]
		;
...
/** @name Arithmetic assignment operators */ /// @{
	...
	inline TMat2x4<T>& operator+=(TMat2x4<T>& m, U const& s) {
		m.data[0] += T(s);
		m.data[1] += T(s);
		return m;
	}
	...
	/// Add matrix.
	template<class T, class U>
	inline TMat2x4<T>& operator+=(TMat2x4<T>& m, TMat2x4<U> const& n) {
		m.data[0] += n.data[0];
		m.data[1] += n.data[1];
		return m;
	}
...
/** @name Increment and decrement operators */ /// @{
	/// Prefix increment.
	template<class T>
	inline TMat2x4<T>& operator++(TMat2x4<T>& m) {
		++m.data[0];
		++m.data[1];
		return m;
	}
	...
	template<class T>
	inline TMat2x4<T> operator++(TMat2x4<T>& m, signed) {
		TMat2x4<T> c{m};
		++m;
		return c;
	}
...
/** @name Unary operators */ /// @{
	/// Matrix unary plus.
	template<class T>
	inline TMat2x4<T> operator+(TMat2x4<T> const& m) {
		return TMat2x4<T>{m};
	}
...
/** @name Arithmetic operators */ /// @{
	...
	template<class T>
	inline TMat2x4<T> operator+(TMat2x4<T> const& m, T const& s) {
		return TMat2x4<T>::operations::scalar_add(m, s);
	}

	/// Matrix left-hand value addition (component-wise).
	template<class T>
	inline TMat2x4<T> operator+(T const& s, TMat2x4<T> const& m) {
		return TMat2x4<T>::operations::scalar_add(m, s);
	}

	/// Matrix addition.
	template<class T>
	inline TMat2x4<T> operator+(TMat2x4<T> const& m, TMat2x4<T> const& n) {
	...
	operator*(TMat2x4<T> const& m, typename TMat2x4<T>::row_type const& v) {
		return TMat2x4<T>::operations::row_multiply(m, v);
	}

	/// Matrix left-hand (column) vector multiplication (proper product).
	template<class T>
	inline typename TMat2x4<T>::row_type
	operator*(typename TMat2x4<T>::col_type const& v, TMat2x4<T> const& m) {
		return TMat2x4<T>::operations::col_multiply(m, v);
	}

	/// Matrix multiplication (proper product).
	template<class T>
	inline TMat2x4<T> operator*(TMat2x4<T> const& m, TMat2x2<T> const& n) {
		return TMat2x4<T>::operations::multiply(m, n);
	}

	/// Matrix multiplication (proper product).
	template<class T>
	inline TMat3x4<T> operator*(TMat2x4<T> const& m, TMat3x2<T> const& n) {
		return TMat2x4<T>::operations::multiply(m, n);
	}

	/// Matrix multiplication (proper product).
	template<class T>
	inline TMat4x4<T> operator*(TMat2x4<T> const& m, TMat4x2<T> const& n) {
		return TMat2x4<T>::operations::multiply(m, n);
	}
