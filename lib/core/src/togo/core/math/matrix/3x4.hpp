#line 2 "togo/core/math/vector/2x4.hpp"
/**
@copyright MIT license; see @ref index or the accompanying LICENSE file.

@file
@brief 3-by-4 matrix interface.
...
struct TMat3x4<T>::operations {
	...
	static TMat2x4<T> multiply(type_cref m, TMat2x3<T> const& n) {
		return TMat2x4<T>{
	...
	static TMat3x4<T> multiply(type_cref m, TMat3x3<T> const& n) {
		return TMat3x4<T>{
			m.data[0].x * n.data[0].x + m.data[1].x * n.data[0].y + m.data[2].x * n.data[0].z,
			m.data[0].y * n.data[0].x + m.data[1].y * n.data[0].y + m.data[2].y * n.data[0].z,
			m.data[0].z * n.data[0].x + m.data[1].z * n.data[0].y + m.data[2].z * n.data[0].z,
	...
	static TMat4x4<T> multiply(type_cref m, TMat4x3<T> const& n) {
		return TMat4x4<T>{
			m.data[0].x * n.data[0].x + m.data[1].x * n.data[0].y + m.data[2].x * n.data[0].z,
			m.data[0].y * n.data[0].x + m.data[1].y * n.data[0].y + m.data[2].y * n.data[0].z,
			m.data[0].z * n.data[0].x + m.data[1].z * n.data[0].y + m.data[2].z * n.data[0].z,
...
/** @name Comparison operators */ /// @{
	...
	template<class T>
	inline bool operator==(TMat3x4<T> const& m, TMat3x4<T> const& n) {
		return
			m.data[0] == n.data[0] &&
			m.data[1] == n.data[1] &&
			m.data[2] == n.data[2]
...
/** @name Arithmetic assignment operators */ /// @{
	...
	inline TMat3x4<T>& operator+=(TMat3x4<T>& m, U const& s) {
		m.data[0] += T(s);
		m.data[1] += T(s);
		m.data[2] += T(s);
		return m;
	...
	/// Add matrix.
	template<class T, class U>
	inline TMat3x4<T>& operator+=(TMat3x4<T>& m, TMat3x4<U> const& n) {
		m.data[0] += n.data[0];
		m.data[1] += n.data[1];
		m.data[2] += n.data[2];
		return m;
...
/** @name Increment and decrement operators */ /// @{
	/// Prefix increment.
	template<class T>
	inline TMat3x4<T>& operator++(TMat3x4<T>& m) {
		++m.data[0];
		++m.data[1];
		++m.data[2];
		return m;
	...
	template<class T>
	inline TMat3x4<T> operator++(TMat3x4<T>& m, signed) {
		TMat3x4<T> c{m};
		++m;
		return c;
	}
...
/** @name Unary operators */ /// @{
	/// Matrix unary plus.
	template<class T>
	inline TMat3x4<T> operator+(TMat3x4<T> const& m) {
		return TMat3x4<T>{m};
	}
...
/** @name Arithmetic operators */ /// @{
	...
	template<class T>
	inline TMat3x4<T> operator+(TMat3x4<T> const& m, T const& s) {
		return TMat3x4<T>::operations::scalar_add(m, s);
	}

	/// Matrix left-hand value addition (component-wise).
	template<class T>
	inline TMat3x4<T> operator+(T const& s, TMat3x4<T> const& m) {
		return TMat3x4<T>::operations::scalar_add(m, s);
	}

	/// Matrix addition.
	template<class T>
	inline TMat3x4<T> operator+(TMat3x4<T> const& m, TMat3x4<T> const& n) {
	...
	operator*(TMat3x4<T> const& m, typename TMat3x4<T>::row_type const& v) {
	...
	operator*(typename TMat3x4<T>::col_type const& v, TMat3x4<T> const& m) {
		return TMat3x4<T>::operations::col_multiply(m, v);
	}

	/// Matrix multiplication (proper product).
	template<class T>
	inline TMat2x4<T> operator*(TMat3x4<T> const& m, TMat2x3<T> const& n) {
		return TMat3x4<T>::operations::multiply(m, n);
	}

	/// Matrix multiplication (proper product).
	template<class T>
	inline TMat3x4<T> operator*(TMat3x4<T> const& m, TMat3x3<T> const& n) {
		return TMat3x4<T>::operations::multiply(m, n);
	}

	/// Matrix multiplication (proper product).
	template<class T>
	inline TMat4x4<T> operator*(TMat3x4<T> const& m, TMat4x3<T> const& n) {
		return TMat3x4<T>::operations::multiply(m, n);
	}
