#line 2 "togo/core/math/vector/3x2.hpp"
/**
@copyright MIT license; see @ref index or the accompanying LICENSE file.

@file
@brief 3-by-2 matrix interface.
...
/** @cond INTERNAL */
template<class T>
struct TMat3x2<T>::operations {
	using type = typename TMat3x2<T>::type;
	...
	static TMat2x2<T> multiply(type_cref m, TMat2x3<T> const& n) {
		return TMat2x2<T>{
	...
	static TMat3x2<T> multiply(type_cref m, TMat3x3<T> const& n) {
		return TMat3x2<T>{
			m.data[0].x * n.data[0].x + m.data[1].x * n.data[0].y + m.data[2].x * n.data[0].z,
			m.data[0].y * n.data[0].x + m.data[1].y * n.data[0].y + m.data[2].y * n.data[0].z,
			m.data[0].x * n.data[1].x + m.data[1].x * n.data[1].y + m.data[2].x * n.data[1].z,
	...
	static TMat4x2<T> multiply(type_cref m, TMat4x3<T> const& n) {
		return TMat4x2<T>{
			m.data[0].x * n.data[0].x + m.data[1].x * n.data[0].y + m.data[2].x * n.data[0].z,
			m.data[0].y * n.data[0].x + m.data[1].y * n.data[0].y + m.data[2].y * n.data[0].z,
			m.data[0].x * n.data[1].x + m.data[1].x * n.data[1].y + m.data[2].x * n.data[1].z,
...
/** @name Comparison operators */ /// @{
	...
	template<class T>
	bool operator==(TMat3x2<T> const& m, TMat3x2<T> const& n) {
		return
			m.data[0] == n.data[0] &&
			m.data[1] == n.data[1] &&
			m.data[2] == n.data[2]
...
/** @name Arithmetic assignment operators */ /// @{
	...
	inline TMat3x2<T>& operator+=(TMat3x2<T>& m, U const& s) {
		m.data[0] += T(s);
		m.data[1] += T(s);
		m.data[2] += T(s);
		return m;
	...
	/// Add matrix.
	template<class T, class U>
	inline TMat3x2<T>& operator+=(TMat3x2<T>& m, TMat3x2<U> const& n) {
		m.data[0] += n.data[0];
		m.data[1] += n.data[1];
		m.data[2] += n.data[2];
		return m;
...
/** @name Increment and decrement operators */ /// @{
	/// Prefix increment.
	template<class T>
	inline TMat3x2<T>& operator++(TMat3x2<T>& m) {
		++m.data[0];
		++m.data[1];
		++m.data[2];
		return m;
	...
	template<class T>
	inline TMat3x2<T> operator++(TMat3x2<T>& m, signed) {
		TMat3x2<T> c{m};
		++m;
		return c;
	}
...
/** @name Arithmetic operators */ /// @{
	...
	operator*(TMat3x2<T> const& m, typename TMat3x2<T>::row_type const& v) {
	...
	operator*(typename TMat3x2<T>::col_type const& v, TMat3x2<T> const& m) {
